...
8) configurable build+start command in `additional.cmd` + $PORT variable (we still allocate port even if we don't proxy). Defaults to `docker build -t <project.Name> . && exec docker run -p "$PORT":80 --rm <project.Name>`
